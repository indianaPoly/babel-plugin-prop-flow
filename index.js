  ...
  for (const child of node.children || []) {
    ...
    if (t.isJSXExpressionContainer(child)) {
      ...
      if (t.isLogicalExpression(expr) || t.isConditionalExpression(expr)) {
        const nested = [expr.consequent || expr.right, expr.alternate || expr.left];
      ...
      if (t.isArrowFunctionExpression(expr) && t.isBlockStatement(expr.body) === false) {
...
module.exports = function ({ types: t }) {
  return {
    ...
    post() {
      ...
      const mdPath = path.resolve(process.cwd(), "prop-flow.md");
      fs.writeFileSync(mdPath, mdContent);
      console.log(`✅ prop-flow.md saved to: ${mdPath}`);
